"""
Instagram downloader - специализированный загрузчик для Instagram.
Поддерживает:
- Посты с видео
- Reels
- IGTV
...
class InstagramDownloader:
    ...
    def _base_opts(self) -> dict[str, Any]:
        ...
        # Добавляем cookies если есть (для приватных аккаунтов)
        cookiefile = config.cookiefile_or_none(config.INSTAGRAM_COOKIES_FILE)
        if cookiefile:
    ...
    async def probe(self, url: str) -> dict[str, Any]:
        ...
        # Instagram может возвращать карусели (несколько фото/видео в одном посте)
        if info.get("_type") == "playlist" or info.get("entries"):
            # Берём первое видео из карусели
            entries = info.get("entries", [])
            if entries:
                # Фильтруем только видео (не фото)
                video_entries = [e for e in entries if e.get("vcodec") not in (None, "none")]
                if video_entries:
                    info = video_entries[0]
                else:
                    raise DownloaderError(
                        "В посте нет видео, только фотографии. Отправьте ссылку на пост с видео."
                    )
            else:
                raise DownloaderError("Не удалось получить содержимое поста.")
    ...
    async def download(
        ...
        """
        Скачать видео с Instagram.

        Args:
            url: URL видео/reels/поста
        ...
        if extract_audio:
            ...
            # Для Instagram обычно лучшее качество - это и есть оптимальный вариант
    ...
    def _download_sync(
        ...
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            # Обработка каруселей - берём первое видео
            if info.get("entries"):
                video_entries = [e for e in info["entries"] if e.get("vcodec") not in (None, "none")]
                if video_entries:
                    info = video_entries[0]
                else:
                    info = info["entries"][0]
    ...
    def _map_error(self, exc: Exception) -> DownloaderError:
        ...
        raw = str(exc)
        msg = raw.lower()
        logger.warning("Instagram yt-dlp error: %s", raw)
        ...
        if any(m in msg for m in login_markers):
            return LoginRequiredError(
                "Instagram требует авторизации для этого контента. "
                "Добавьте cookies для Instagram — см. README.md."
            )
        ...
        if any(m in msg for m in unavailable_markers):
            return VideoUnavailableError(
                "Контент недоступен: удалён, приватный аккаунт или гео-блокировка."
            )

        if "timeout" in msg:
            return DownloaderError(
                "Превышено время ожидания ответа от Instagram, попробуйте ещё раз."
            )

        # Общая ошибка
        short = raw.strip().splitlines()[-1][:300]
        return DownloaderError(f"Не удалось скачать Instagram контент. Причина: {short}")
